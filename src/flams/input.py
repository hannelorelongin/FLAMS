...
def parse_args(sys_args) -> argparse.Namespace:
    ...
    _validate_modifications(args.modification)
    _check_out_file(args.output)
    _check_pos_provided(args)
...
def validate_input_fasta(args, fasta, pos) -> Path:
    ...
    """
    _check_fasta(fasta)
    _check_pos_in_range(fasta, pos)
    _check_amino_acid(fasta, pos, _get_intersection(args.modification))
...
def _validate_modifications(modifications):
    ...
    if modifications:
        ...
        _untangle_modifications(modifications)
        # remove duplicate modifications
        modifications = list(set(modifications))
        # check if modification is known to FLAMS
        _check_modification_exist(modifications)
        # check if modifications actually share a carrier amino acid
        _check_modifications_share_carrier(modifications)
        logging.info(f"The set of amino acids capable of carrying all your specified modifications {modifications} was determined as {_get_intersection(modifications)}.")
...
def validate_input_uniprot(args, uniprot, pos) -> Path:
    ...
    """
    _check_data_dir(args)
    protein_file = _get_uniprot_file(args, uniprot)
...
def validate_input_batch(batch) -> Path:
    ...
    """
    _check_batch_file(batch)
    _check_batch_content(batch)
    return batch
...
def _create_args_parser():
    ...
    group.add_argument("--in", dest = "input", type = Path, 
    ...
    group.add_argument("--batch", type = Path,
                       help = "Path to tab seperated input file for batch processing (1st column UniProt ID, 2nd column position). " +
                       "One query (UniProtID + position) per line.",
    ...
    # File arguments
    parser.add_argument("-o", "--output", type = Path, default = Path("out.tsv"),
                        help = "Path to output .tsv file. [default: out.tsv] " + 
                        "If FLAMS is run with --batch, the specified -o/--output is used as preposition, followed by '_$UniProtID_$position.tsv'. [default: '']",
                        metavar = "outputFilePath")

    parser.add_argument("-d", "--data_dir", type = Path, default = Path(os.getcwd()) / "data",
                        help = "Path to directory where intermediate files should be saved. [default: $PWD/data]",
                        metavar = "dataDir")
    ...
    parser.add_argument("-e", "--evalue", type = float, default = 0.01,
                        help = "Desired E-value of BLAST run. [default: 0.01]",
                        metavar = "evalueBLAST")

    parser.add_argument("-m", "--modification", nargs = "+", default = ["K-All"],
                        help = "Space-seperated list of modifications (all lower case) to search for at the given position. " + 
                        "Possible values are any (combinations) of the CPLM, dbPTM and SCOP3P modifications. " +
                        "We also provide aggregated combinations for each amino acid (AA-All), and the CPLM combinations. " + 
                        "For a full list of all supported PTMs, and how they are named, see the Supported PTM types section of the README. " + 
                        "In general, PTMs are written all lowercase, and spaces within a PTM name are replaced by underscores. [default: K-All]",
                        metavar = "modification")

    return parser

# Check functions (order alphabetically)
def _check_amino_acid(fasta, pos, intersection):
    """
    ...
    """
    try:
        if not _is_position_carrier_of_mod(pos, fasta, intersection):
            logging.error(
...
def _check_batch_content(batch):
    """
...
def _check_data_dir(args):
    ...
    if args.data_dir.exists() and not args.data_dir.is_dir():
        logging.error(f"Provided data directory is not a directory: {args.data_dir}. {exit_message}")
        sys.exit()
    ...
    if not args.data_dir.is_dir():
        if args.data_dir.parent.is_dir():
            ...
            logging.error(f"Provided path is not an existing path: {args.data_dir}. {exit_message}")
            sys.exit()

def _check_fasta(fasta):
    """
    ...
    if not _is_valid_fasta_file(fasta):
        logging.error(f"Input file {fasta} is not a valid FASTA file. Exiting FLAMS...")
        sys.exit()

def _check_modification_exist(modifications):
    """
...
def _check_modifications_share_carrier(modifications):
    """
    ...
    """
    if len(_get_intersection(modifications)) == 0:
...
def _check_out_file(output_file):
    """
...
def _check_pos_in_range(fasta, pos):
    """
    ...
    """
    if not _is_within_range(pos, fasta):
        logging.error(f"Please provide a lysine position smaller than the size of your protein ({get_length_protein(fasta)}).")
        sys.exit()

def _check_pos_provided(args):
    """
...
def _is_position_carrier_of_mod(pos: int, fasta: Path, intersection) -> bool:
    """
    This function assess whether the user provided position actually points to an amino acid that is part of the set of amino acids that can carry all user specified modifications.
...
def _is_valid_fasta_file(fasta: Path) -> bool:
    """
...
def _is_within_range(pos: int, fasta: Path) -> bool:
    """
    ...
    # user provides position in 1-based indexing system
    position_idx = pos - 1
    length = get_length_protein(fasta)
    return position_idx < length

# Get functions (ordered alphabetically)
def _get_intersection(modifications):
    """
...
def get_length_protein(fasta: Path) -> int:
    """
...
def _get_uniprot_file(args, uniprot) -> Path:
    """
    ...
    try:
        return _retrieve_protein_from_uniprot(args, uniprot)
...
# Retrieve function
def _retrieve_protein_from_uniprot(args, uniprot) -> Path:
    """
...
# Other Functions
def _untangle_modifications(modifications):
    """
